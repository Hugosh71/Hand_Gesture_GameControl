...
TRIGGER_GESTURE = "two_up"   # geste qui déclenche le saut (change si tu veux)
KEY_TO_PRESS = "space"       # Flappy: souvent SPACE (sinon "up" selon versions)
COOLDOWN_SEC = 0.25          # temps minimum entre deux sauts
CONF_THRESH = 0.85           # seuil de confiance
MARGIN_THRESH = 0.25         # marge top1 - top2
SMOOTH_N = 9                 # vote sur 9 frames

# Sécurité: bouger la souris en haut à gauche stoppe pyautogui
...
def main():
    ...
    print("Contrôle Flappy Bird")
    print(" - Appuie sur A pour ARM/DISARM (envoyer SPACE)")
    print(" - Appuie sur Q pour quitter")
    print("Astuce: clique sur la fenêtre du jeu pour lui donner le focus, puis appuie sur A.")
    ...
    while True:
        ...
        # ===== Trigger: uniquement sur "front montant" (no -> gesture) + cooldown =====
        now = time.time()
        rising_edge = (label == TRIGGER_GESTURE) and (last_label != TRIGGER_GESTURE)
        can_send = (now - last_sent) >= COOLDOWN_SEC

        if armed and rising_edge and can_send:
        ...
        txt = f"{status} | label={label} (raw={raw_label} conf={conf:.2f}) | FPS={fps:.1f}"
        cv2.putText(frame, txt, (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.70, (0, 0, 0), 4, cv2.LINE_AA)
        cv2.putText(frame, txt, (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.70, (255, 255, 255), 1, cv2.LINE_AA)
        cv2.putText(frame, "A: arm/disarm | Q: quit", (20, 70),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.65, (255, 255, 255), 1, cv2.LINE_AA)

        cv2.imshow("Flappy controller", frame)
        ...
        if k in [ord("a"), ord("A")]:
            armed = not armed
            # petit "debounce" pour éviter double toggle
            time.sleep(0.2)
